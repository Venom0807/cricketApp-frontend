    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="container max-w-md mx-auto">
        <form onSubmit={onSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
          ...
          <button disabled={loading} className="w-full px-4 py-2 rounded bg-blue-600 text-white">{loading ? 'Signing in...' : 'Login'}</button>
          <div className="text-center text-sm">
            <Link to="/register" className="text-blue-600">Register Captain</Link>
          </div>
        </form>
      </div>
    </div>
